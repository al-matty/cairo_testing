...
mod tests {
    ...
    fn test_constructor(){
        ...
        let symbol: felt252 = 'mad_coin';

        My_token::constructor(name, symbol, decimals, initial_supply, account);

        // Testing initialized variables
        assert(My_token::get_name() == name, 'erc20: wrong name'); 
        assert(My_token::get_decimals() == decimals, 'erc20: incorrect decimals');
        assert(My_token::get_symbol() == symbol, 'erc20: wrong symbol');
        assert(My_token::get_total_supply() == initial_supply, 'erc20: wrong supply');
    }
